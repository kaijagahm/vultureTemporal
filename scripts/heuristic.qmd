...
What would happen if we selected the same number of time slices for each one?

```{r}
# select 5 slices from each, many times
cvs_5 <- measures %>% 
  group_by(w, situ) %>%
  sample_n(5) %>%
  group_by(w, situ) %>%
...
reps100 <- map(1:100, ~{
  measures %>% group_by(w, situ) %>% sample_n(5) %>% group_by(w, situ) %>% summarize(cv = mean(density, na.rm = T)/sd(density, na.rm = T)) %>% ungroup() %>% mutate(w_days = w)
})

reps <- data.table::rbindlist(reps100, idcol = "rep")
bars <- reps %>% group_by(w_days, situ) %>%
  summarize(mn = mean(cv), sd = sd(cv))

reps %>%
  ggplot(aes(x = factor(w_days), y = cv, col = situ))+
  geom_boxplot()

reps %>%
  ggplot()+
  geom_jitter(alpha = 0.5, pch = 1, aes(x = w_days, y = cv, col = situ))+
  ylab("CV")+
...
cvs_fig <- cvs %>%
  ggplot(aes(x = w_days, y = cv/(122/w), col = situ))+
