...
curves_seasons <- curves_seasons %>%
  group_by(situ) %>%
  mutate(ent_norm = ent/max(ent, na.rm = T))
curves_seasons_subset <- curves_seasons_subset %>%
  group_by(situ) %>%
  mutate(ent_norm = ent/max(ent, na.rm = T))
...
plt_curves_seasons_subset <- curves_seasons_subset %>%
  ggplot(aes(x = step, y = ent_norm, col = situ))+
  annotate(geom = "segment", x = 1, xend = 9, y = 1, yend = 0, alpha = 0.7, col = "black", linetype = 3)+
  geom_point(size = 2, alpha = 0.7)+
  geom_line(linewidth = 1, alpha = 0.7, aes(linetype = situ))+
...
ggsave(plt_curves_seasons_subset, file = here("fig/plt_curves_seasons_subset.png"), width = 6, height = 3)
...
mat_fe_subset <- provideDimnames(as.matrix(red_feeding_cat_seasons_subset$JSD), sep = "",
                          list(season_names, season_names))
mat_fl_subset <- provideDimnames(as.matrix(red_flight_cat_seasons_subset$JSD), sep = "",
                          list(season_names, season_names))
...
## automatic (flight)
gplots::heatmap.2(mat_fl, trace = "none", dendrogram = "row", key = FALSE, margins = c(10, 10), main = "124 indivs (flight)")
gplots::heatmap.2(mat_fl_subset, trace = "none", dendrogram = "row", key = FALSE, margins = c(10, 10), main = "20 indivs (flight)")
...
# --- Heatmap 1: Original with dendrogram (as per reprex) ---
gplots::heatmap.2(mat_fl, trace = "none", dendrogram = "row", key = FALSE,
                  margins = c(10, 10))

gplots::heatmap.2(mat_fl_subset, trace = "none", dendrogram = "row", key = FALSE,
                  margins = c(10, 10))
...
gplots::heatmap.2(mat_fl_subset, trace = "none",
                  Rowv = dendextend::rotate(as.dendrogram(hclust(as.dist(mat_fl_subset))), order = season_names_abbreviated),
                  Colv = dendextend::rotate(as.dendrogram(hclust(as.dist(mat_fl_subset))), order = season_names_abbreviated),
                  dendrogram = "column", 
                  key = TRUE, margins = c(10, 10), density.info = "none", key.title = "", key.xlab= "", key.ylab = "", keysize = 2)
...
# gplots::heatmap.2(mat_fe_subset, trace = "none",
#                   Rowv = dendextend::rotate(as.dendrogram(hclust(as.dist(mat_fe_subset))), order = season_names_abbreviated),
#                   Colv = dendextend::rotate(as.dendrogram(hclust(as.dist(mat_fe_subset))), order = season_names_abbreviated),
#                   dendrogram = "column", 
#                   key = TRUE, margins = c(10, 10), density.info = "none", key.title = "", key.xlab= "", key.ylab = "", keysize = 2)
...
gplots::heatmap.2(mat_fl_subset[rev(season_names_abbreviated), season_names_abbreviated], 
                  trace = "none",
                  dendrogram = "none", # No dendrogram
                  key = FALSE,
                  Rowv = NA,           # Prevent further reordering by Rowv
...
# gplots::heatmap.2(mat_fe_subset[rev(season_names_abbreviated), season_names_abbreviated], 
#                   trace = "none",
#                   dendrogram = "none", # No dendrogram
#                   key = FALSE,
#                   Rowv = NA,           # Prevent further reordering by Rowv
...
gplots::heatmap.2(mat_fl_subset[rev(season_order), season_order], 
                  trace = "none",
                  dendrogram = "none", # No dendrogram
                  key = FALSE,
                  Rowv = NA,           # Prevent further reordering by Rowv
...
# gplots::heatmap.2(mat_fe_subset[rev(season_order), season_order], 
#                   trace = "none",
#                   dendrogram = "none", # No dendrogram
#                   key = FALSE,
#                   Rowv = NA,           # Prevent further reordering by Rowv
...
# 2025-09-17 edit: re-did analysis with the 20-vulture subset, as shown above.
